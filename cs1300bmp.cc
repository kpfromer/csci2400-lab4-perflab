...
static bool bmp_read(char *file_in_name, unsigned long int *width,
                     long int *height, unsigned char **rarray,
                     unsigned char **garray, unsigned char **barray);

static bool bmp_24_write(char *file_out_name, unsigned long int width,
                         long int height, unsigned char *rarray,
                         unsigned char *garray, unsigned char *barray);
...
//
{
  ...
  //
  //  If you are doing swapping, you have to reunswap the filetype, I think, JVB
  //  15 December 2004.
  //
  if (bmp_byte_swap) {
...
bool bmp_read(char *file_in_name, unsigned long int *width, long int *height,
              unsigned char **rarray, unsigned char **garray,
...
int cs1300bmp_readfile(char *filename, struct cs1300bmp *image) {
  ...
  //
  error = bmp_read(filename, &width, &height, &rarray, &garray, &barray);
  if (error) {
    //
    //  Free the memory.
    //
...
int cs1300bmp_writefile(char *filename, struct cs1300bmp *image) {
  ...
  int error = bmp_24_write(filename, image->width, image->height, rarray,
                           garray, barray);
